...
export async function getThreadById(req: Request, res: Response) {
  const { id } = req.params;
  const userId = (req as any).user.id; // Ambil ID pengguna yang login
...
export async function getThreadByUser(req: Request, res: Response) {
  const userId = (req as any).user.id; // Ambil id dari pengguna yang login
...
export async function getAllThreads(req: Request, res: Response) {
  ...
  try {
    ...
    // Tambahkan status like untuk setiap thread
    const threadsWithLikeStatus = await Promise.all(
    ...
    if (threadExist.image) {
      ...
      if (publicId) {
        ...
        await cloudinary.uploader.rename(
          publicId,
          `threads_deleted/${publicId.split('/').pop()}`,
        );
      }
    }
...
export async function getAllThreadByAuthorId(req: Request, res: Response) {
